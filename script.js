    ...
    init(){//position each block in its proper position
        for(let y=0;y<this.rows;y++){
            for(let x=0;x<this.cols;x++){
                ...
                block.addEventListener('mouseover',(e)=>this.onClickOnBlock(blockIdx));
...
// for get random color in anytime that the page is refresh

window.onload = function() {

    let blocchi = document.getElementById("puzzle_container");
    ...
    r = Math.floor(Math.random() * (max - min + + min) ) + min;
    g = Math.floor(Math.random() * (max - min + + min) ) + min;
    b = Math.floor(Math.random() * (max - min + + min) ) + min;

    blocchi.style.background = "rgb(" + r + ", " + g + ", " + b + ")";
}


//clock 

let buottone = document.querySelector('.difficulty_button active');

buottone.addEventListener('click', start);
...
let soggetto = document.querySelector(".puzzle_block");
let testo = document.getElementById("move");
let mosse = 0;

soggetto.addEventListener('mouseover', yeah);

function yeah() { 


testo.innerHTML = "move: " + mosse;
mosse++;
